...
export class BoardService {
  ...
  async getAll(dto: QueryBoardParamsDto) {
    const { page = 1, limit = 4, titles } = dto;
    try {
      ...
      if (titles) {
        ...
        queryBuilder = queryBuilder
          .andWhere('board.title IN (:...titleNames)', { titleNames: titlesArray }); // Виправлено: використано board.title замість title.name
      }
  ...
  async getAllTitles() {
    try {
      const titlesWithIds = await this.boardRepository.createQueryBuilder('board')
